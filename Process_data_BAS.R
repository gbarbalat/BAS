...
# attrition ---- 
# compare included (merged_ignore) - full sample; calculate attrition weights if necessary
#run current script except that no filtering (on full sample)
source("zombie_process_data_for_full_sample.R")
merged_ignore_full <- zombie_for_full()
names(length_Exp_preN) <- sub("_.*", "", names(length_Exp_preN))
length_Exp_preN <- length_Exp_preN[!duplicated(names(length_Exp_preN))]
merged_ignore <- merged_ignore %>%
  mutate(GEST_AGE = length_Exp_preN[as.character(id_Dem806_585_GB)])
merged_ignore_full <- merged_ignore_full %>%
  mutate(GEST_AGE = length_Exp_preN[as.character(id_Dem806_585_GB)])
source("compare_inc_full.R")#includes comparisons of GEST_AGE
save(table1, file=paste0(Exp,"_table1_inc_full.RData"))
...
# remove id col
merged_imputed <- mice(merged_ignore %>% select(-id_Dem806_585_GB), m=m, maxit = maxit, print = F)
...
#fmi-lambda
predictors <- paste(merged_ignore %>% dplyr::select(- ALL, -M02R_DEMENAG, - id_Dem806_585_GB) %>% colnames, collapse = "+")
fit_MV <- with(merged_imputed, lm(formula=formula(paste("ALL ~ ", predictors))))
pooled <- pool(fit_MV)
fmi_lambda <- pooled$pooled %>% select(term, lambda, fmi)
fmi_lambda

# Table1Imputed/NonImputed
#compare merged_ignore and merge_imputed
source("compare_imp_nonimp.R")
save(table1, file=paste0(Exp,"_table1_imp_nonimp.RData"))

# pre-anal ----
# BiV, multivar C/S, lgtd
# BiV
predictors <- setdiff(names(merged_ignore), c("ALL","M02R_DEMENAG", "id_Dem806_585_GB"))
# Store models in a list
BiV_results <- list()
for (pred in predictors) {
  formula_str <- paste("ALL ~", pred)
  fit_BiV <- with(merged_imputed, lm(formula = as.formula(formula_str)))
  pooled <- pool(fit_BiV)
  BiV_results[[pred]] <- summary(pooled)
}
BiV_results[["GEST_AGE"]]#preN_count_above_90 preN_count_above_95 preN_consec_count_above_90 preN_consec_count_above_95
# multivariate analysis
summary_fit_MV <- summary(fit_MV)

#add id to merged_imputed
merged_imputed$id <- merged_ignore$id_Dem806_585_GB
save(merged_imputed, file = paste0(Exp, "_merged_imputed.RData" ))
...
source("Process_data_BAS_CF.R")
Exp_CF1 <- create_db_policy(id=merged_ignore$id_Dem806_585_GB, Exp, POLICY=1, p90, p95); save(Exp_CF1, file=paste0(Exp,"_Exp_CF1.RData"))
Exp_CF2 <- create_db_policy(id=merged_ignore$id_Dem806_585_GB,Exp, POLICY=2, p90, p95); save(Exp_CF2, file=paste0(Exp,"_Exp_CF2.RData"))
Exp_CF3 <- create_db_policy(id=merged_ignore$id_Dem806_585_GB,Exp, POLICY=3, p90, p95); save(Exp_CF3, file=paste0(Exp,"_Exp_CF3.RData"))