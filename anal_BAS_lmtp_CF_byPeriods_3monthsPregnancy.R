...
#/*
all_period <- c("preN1", "preN2", "preN3", "postN1", "postN2", "postN3")
#*/
...
#/*
trt <- c(paste0("preN1_", trt_suffix),
         paste0("preN2_", trt_suffix),
         paste0("preN3_", trt_suffix),
...
#/*
names(length_Exp_preN3) <- sub("_.*", "", names(length_Exp_preN3))
length_Exp_preN3 <- length_Exp_preN3[!duplicated(names(length_Exp_preN3))]
#*/
...
Exp_as_pct <- function(which_df) {
  which_df <- which_df %>%
    mutate(
      # divide preN1_ columns by 13*7
      across(starts_with("preN1_"),
             ~ .x / 13*7),
      
      # divide preN2_ columns by 14*7
      across(starts_with("preN2_"),
             ~ .x / 14*7),
      
      # divide preN3_ columns by (length_Exp_preN3[id])
      across(starts_with("preN3_"),
             ~ .x / (length_Exp_preN3[as.character(id)])),
...
run_lmtp_imputed_df <- function(i) {
  ...
  #/*
  # add on df and cold preN1, preN2, preN3
  merged_imputed_thisone <- merged_imputed_thisone %>%
    left_join(df_preN1, by = c("id" = "group")) %>%
    left_join(df_preN2, by = c("id" = "group")) %>%
    left_join(df_preN3, by = c("id" = "group")) %>%
    left_join(cold_preN1, by = c("id" = "group")) %>%
    left_join(cold_preN2, by = c("id" = "group")) %>%
    left_join(cold_preN3, by = c("id" = "group")) 
