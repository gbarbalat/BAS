...
load (file = paste0(Exp, "_merged_imputed_IPCW.RData" ))
...
lmtp_sdr(
  ...
  cens = c("C_preN","C_postN1","C_postN2","C_postN3"),
  shift = shift,
  shifted = shifted,
  k = Inf,
  mtp = mtp,
...
run_lmtp_imputed_df <- function(i) {
  ...
  merged_imputed_thisone <- merged_imputed_thisone %>%
    left_join(merged_ignore_with_id_Cs %>% select(c(id_Dem806_585_GB, all_of(starts_with("C_", ignore.case = F)))), 
              by = c("id" = "id_Dem806_585_GB"))
  
  # 1. Define the vectors of column names to mask for each step
  cols_postN1 <- c(trt[2], time_vary$postN1)
  cols_postN2 <- c(trt[3], time_vary$postN2)
  cols_postN3 <- c(trt[4], time_vary$postN3)
  cols_outcome <- "ALL"
  
  # 2. Step 1: Where C_preN == 0, set postN1 variables to NA
  # (Using C_0 as a fallback if your variable is named C_0 instead of C_preN)
  preN_condition <- if ("C_preN" %in% colnames(merged_imputed_thisone)) {
    merged_imputed_thisone$C_preN == 0
  } else {
    merged_imputed_thisone$C_0 == 0
  }
  merged_imputed_thisone[preN_condition, cols_postN1] <- NA
  
  # 3. Step 2: Where C_postN1 == 0, set postN2 variables to NA
  merged_imputed_thisone[merged_imputed_thisone$C_postN1 == 0, cols_postN2] <- NA
  
  # 4. Step 3: Where C_postN2 == 0, set postN3 variables to NA
  merged_imputed_thisone[merged_imputed_thisone$C_postN2 == 0, cols_postN3] <- NA
  
  # 5. Step 4: Where C_postN3 == 0, set outcome "ALL" to NA
  merged_imputed_thisone[merged_imputed_thisone$C_postN3 == 0, cols_outcome] <- NA
  
  
  #CF df
  # 1. Dynamically retrieve the reference data frame using get()
  exp_df <- get(paste0("Exp_", CF))
  exp_df <- as.data.frame(t(unique(t(exp_df))))
  
  # 2. Prepare exp_df so it only contains the key and the trt columns, 
  # and rename 'group' to 'id' so it matches your main data frame
  exp_subset <- exp_df %>%
    rename(id = group) %>%
    select(id, all_of(trt))
  
  # 3. Update only those matching rows and trt columns in your main data frame
  merged_imputed_CF <- merged_imputed_thisone %>%
    rows_update(exp_subset, by = "id", unmatched = "ignore") %>%
    mutate(C_preN=1,C_postN1=1, C_postN2=1, C_postN3=1)
